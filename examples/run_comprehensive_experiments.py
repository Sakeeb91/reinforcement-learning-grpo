#!/usr/bin/env python3
"""
Run Comprehensive GRPO Hospital Experiments - Production version
"""
...
def train_single_experiment(
    ...
    num_episodes: int = 400  # Balanced for good results and reasonable runtime
) -> Dict[str, Any]:
    ...
    env = HospitalEnv(
        ...
        episode_length=250,  # Longer episodes for more realistic data
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 50 == 0:
            ...
            recent_fairness = np.mean(fairness_scores[-25:])
    ...
    for eval_episode in range(20):  # Comprehensive evaluation
        state = env.reset()
        eval_reward = 0
        eval_info = []
    ...
    final_results = {
        ...
        'detailed_metrics_history': detailed_metrics_history[-50:],  # Last 50 episodes
        
        # Final training performance
        'final_training_reward_mean': np.mean(episode_rewards[-50:]),
        'final_training_reward_std': np.std(episode_rewards[-50:]),
        'final_training_fairness_mean': np.mean(fairness_scores[-50:]),
        'final_training_fairness_std': np.std(fairness_scores[-50:]),
...
def run_comprehensive_experiments():
    ...
    num_episodes = 400  # Good balance of training and runtime
    ...
    print(f"🕒 Estimated total runtime: {len(experiments) * num_episodes * 0.3 / 60:.1f} minutes")
    print()
    
    # Determine parallel processing
    num_processes = min(3, mp.cpu_count())  # Conservative to avoid overwhelming system
...
def save_experiment_results(results: List[Dict], save_dir: str = "comprehensive_results"):
    ...
    pickle_path = os.path.join(save_dir, f"hospital_experiments_comprehensive_{timestamp}.pkl")
    ...
    for result in results:
        summary = {
            ...
            'reward_improvement': result['learning_convergence']['reward_improvement'],
            'fairness_improvement': result['learning_convergence']['fairness_improvement'],
            ...
            if 'wait_times_by_demographic' in demo_metrics:
                for demo, stats in demo_metrics['wait_times_by_demographic'].items():
                    if stats:
                        ...
                        base_row[f'{demo}_wait_median'] = stats['median']
                        base_row[f'{demo}_patients_count'] = stats['count']
            
            if 'resource_utilization_stats' in demo_metrics:
                util_stats = demo_metrics['resource_utilization_stats']
                base_row['bed_utilization_mean'] = util_stats['beds']['mean']
                base_row['staff_utilization_mean'] = util_stats['staff']['mean']
                base_row['equipment_utilization_mean'] = util_stats['equipment']['mean']
            
            if 'efficiency_metrics' in demo_metrics:
                eff_metrics = demo_metrics['efficiency_metrics']
                base_row['total_patients_served'] = eff_metrics['total_patients_served']
                base_row['throughput_rate'] = eff_metrics['throughput_rate']
            
            if 'equity_metrics' in demo_metrics:
                equity_metrics = demo_metrics['equity_metrics']
                base_row['wait_time_coefficient_variation'] = equity_metrics['wait_time_coefficient_variation']
                base_row['demographic_parity_score'] = equity_metrics['demographic_parity_score']
...
def analyze_experiment_results(results: List[Dict]):
    ...
    print("• Comprehensive data suitable for professional healthcare AI portfolio")
...
if __name__ == "__main__":
    ...
    print("Data is now available for professional visualizations and analysis.")